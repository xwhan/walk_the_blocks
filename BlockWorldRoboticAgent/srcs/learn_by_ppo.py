...
def ppo_update(agent, sl_path):
	...
	for epoch in range(args.max_epochs):
		...
		for sample_id in tqdm(range(dataset_size)):
			...
			for _ in range(args.ppo_epoch):
				final_loss = agent.policy_model.ppo_loss(batch, old_model, rewards, baselines, args)
				opti.zero_grad()
				final_loss.backward()
	...
	save_path = '../models/ppo_critic_epochs' + str(args.max_epochs) + '_lr_' + str(args.lr) + '.pth'
