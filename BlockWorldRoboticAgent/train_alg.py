...
def advesarial_imitation(agent):
	...
	for epoch in range(args.max_epochs):
		...
		for sample_id in tqdm(range(dataset_size)):
			...
			block_id_pred = agent.sample_policy(block_prob, method='greedy')
			...
			for exp in expert_memory:
				...
				expert_path.append((deepcopy(state), direction_id))
			...
			policy_value = values_selected.mean().data.cpu().numpy()
			...
			expert_value = values_selected.mean().data.cpu().numpy()
			loss_expert = torch.log(1 - values_selected + 1e-8).mean()
			d_loss =  - (loss_expert + loss_policy)

			log_value('values of policy', policy_value, sample_id)
			log_value('values of expert', expert_value, sample_id)

			opti_critic.zero_grad()
			d_loss.backward()
			# torch.nn.utils.clip_grad_norm(agent.critic_model.parameters(), 5.0)
			opti_critic.step()
			...
			for _ in range(args.ppo_epoch):
				...
				gather_indices = torch.arange(0, batch_size).long().cuda() * batch_size + policy_batch[4].data
				values = torch.log(values.view(-1).index_select(0, Variable(gather_indices)) + 1e-8) # cost
